<template>
  <div>
    <ul class="pagination pagination-sm" v-if="pageArray.length > 0">
      <li class="page-item"><a href="#" :class="{'disabled-menu': currentPage == 1}" @click.prevent="controlPage(pageArray[0].page_no)"><i class="fas fa-angle-double-left"></i></a></li>
      ...
      <li class="page-item"><a href="#" :class="{'disabled-menu': getTotalPages() == currentPage}" @click.prevent="controlPage(pageArray[pageArray.length - 1].page_no)"><i class="fas fa-angle-double-right"></i></a></li>
    </ul>
  </div>
</template>
...
<script>
  let cpn = {
    ...
    methods: {
      ...
      createPagesArray() {
        this.pageArray = this.page.createPagesArray()
      },
      changePage(pn) {
