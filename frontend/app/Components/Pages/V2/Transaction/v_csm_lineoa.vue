  ...
  export default {
    ...
    methods: {
      ...
      before_addedFile() {
        ...
        this.form_beforepicsAttach = []
        this.$refs.modal_addList.openModal()
      },
      openEdit(idx) {
        this.isEditMode = true
        this.editIdx = idx
        this.form.subject = this.form_picsAttach[idx].subject
        this.form.description = this.form_picsAttach[idx].description
        this.form_beforepicsAttach = [...this.form_picsAttach[idx].picsAttach]
        this.$refs.modal_addList.openModal()
      },
      Confirm_editFile() {
        ...
        this.form_picsAttach[this.editIdx].subject = this.form.subject
        this.form_picsAttach[this.editIdx].description = this.form.description
        this.form_picsAttach[this.editIdx].picsAttach = [...this.form_beforepicsAttach]
        this.isEditMode = false
        this.editIdx = null
        this.form.subject = ""
        this.form.description = ""
      ...
      async isMember() {
        ...
        if (this.customer.type == "line_member") {
          this.tabs[6].showbutton = false
          this.tabs[7].showbutton = false
      ...
      async Tracking_ReadList(keyword) {
        ...
        switch (keyword) {
          case "N":
            ...
            this.chkstatus = resp.data.data
            this.List_status.N = resp.data.total.N
            this.List_status.R = resp.data.total.R
            this.List_status.P = resp.data.total.P
            this.List_status.Y = resp.data.total.Y
            this.List_status.HW = resp.data.total.HW
            this.List_status.All = resp.data.total.All
            //await this.viewImagex();

            // this.chkstatusDisplay = resp.data.data
      ...
      async onRead(obj) {
        ...
        if (resp.data.length > 0) {
          this.form_h = d
          this.form.pre_event = d.pre_event
          this.form.contract_tel = d.contract_tel
          this.form.projrunno = d.projrunno
          this.form.maincode = d.maincode
          this.form.customer_code = d.customer_code
          this.member_obj = d
          this.form.map_url = d.map_url
          this.form.map_gps = d.map_gps
          this.form.map_desc = d.map_desc
      ...
      async Description_Read(obj) {
        ...
        resp.data.data.date_convenient1 = moment(resp.data.data.date_convenient1).isValid() ? moment(resp.data.data.date_convenient1).format('DD/MM/YYYY') : ''
        resp.data.data.date_convenient2 = moment(resp.data.data.date_convenient2).isValid() ? moment(resp.data.data.date_convenient2).format('DD/MM/YYYY') : ''

        this.chkstatus_detail = resp.data.data
      },

      async Score_Read(text, obj) {
        let action = `CSM/CustomerData/Score_Read?maincode=${obj.maincode}&csmno=${obj.job_no}`
        let resp = await $xt.getCustomerServer(action);
        this.score_ans = $linq(resp.data.data).sum(x => x.score_ans)
        this.score = resp.data.data
        this.score_remark = this.score[0]?.remark || ''
        this.score_obj = obj
      ...
      async MemberRequest_Read() {
        ...
        let resp = await $xt.getCustomerServer(action);
        this.member_req = resp.data
        this.tabs[7].total = resp.total
        loading.hide()
      },
      async readImg() {
        let act = `CSM/API/csm_img_bg`;
        let resp = await $xt.getServer(act);
        this.img_bg = resp.data || [];
      ...
      async SwitchContent(keyword, x) {
        switch (keyword) {
          case "tab2_form":
            ...
            this.form.contract = this.customer.customer_name
      ...
      onReset(keyword) {
        switch (keyword) {
          ...
          case "cancel_addFile":
            ...
            break
          case "cancel_vote":
            this.selectedTabs.rating = false
            this.$refs.modal_vote.closeModal()
            break
        }
      },
      changeheader() {
        this.form.pre_event = this.form_h.pre_event
        this.form.contract_tel = this.form_h.contract_tel
        this.form.projrunno = this.form_h.projrunno
        this.form.maincode = this.form_h.maincode
        this.form.address = this.form_h.address
        this.form.customer_code = this.form_h.customer_code
      },
      beforeaddFile(type) {
        if (type === 'picture') {
      ...
      addScore(keyword, idx, score) {
        switch (keyword) {
          case "score":
            this.score[idx].score_ans = score

            break;
          case "over":
            this.score[idx].score_hover = score
            break;
        }
      },
      async SaveScore() {
        ...
        this.score.forEach((item, index) => {
          ...
          item.job_no = this.score_obj.job_no
          item.adduser = this.customer.userid
        })
        if (valid.length > 0) {
          $msg.alert(this.ui.csm_v2_warning, valid, 'warning')
          return
        }
        this.score[0].remark = this.score_remark || ''
        try {
          loading.show()
          let form = {
            data: this.score,
      ...
      Openmodal_addMember() {
        ...
        // this.form_member_proj = proj
        this.form_member_proj = this.member_obj
        this.form_member = {}
      ...
      createUrlTemplate(data) {
        switch (data) {
          ...
          case 'copy':
            ...
            document.execCommand('copy');
            document.body.removeChild(tempTextArea);
            this.url_create = this.copyUrl_login;
            break;
      ...
      generateRandomPassword() {
        ...
        this.form_member.userpass = password
      },
      ...
      onlyPressNumber2() {
        ...
        this.form_member.telephone = this.form_member.telephone.replace(/[^0-9]/g, '')
      ...
      filterInput(event, keyword) {
        switch (keyword) {
          case "add_member":
            this.form_member.userid = event.target.value.replace(/[^a-z0-9]/gi, '')
            break;
      ...
      sendComponent(e, type) {
        switch (type) {
          case 'description':
            ...
            this.form.note = e.desname
            // this.closeDescModal()
            break
          case 'description2':
            this.form.description = e.desname
            // this.closeDescModal()
            break
          case 'subject':
            this.form.subject = e.desname
            // this.closeDescModal()
            break
        }
      },
      ...
      pageChange(pn, pt) {
        ...
        this.chkstatusDisplay = $linq(this.chkstatus).skip(paging.skipItems()).take(paging.getItemsPerPage()).toArray();
        paging.createPagesArray();
      },
      ...
      async showCommentExt(job_no, ref_docno) {
        ...
        try {
          ...
          let resp = await $xt.getCustomerServer(action)
          this.data_comment = resp.commentData
      ...
      async fetchSubProjects(projectId) {
        ...
        try {
          ...
          if (resp.data) {
            ...
            this.loadedPhases[projectId] = phases;
          }
        } catch (error) {
