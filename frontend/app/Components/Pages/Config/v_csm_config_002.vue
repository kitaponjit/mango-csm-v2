                        ...
                        <table class="table table-bordered table-hover">
                          <thead>
                            <tr>
                              ...
                              <th class="tf-3 text-center">State Code</th>
                              <th>State Name</th>
                          ...
                          <tbody>
                            <tr v-for="x,idx in extensionData">
                              ...
                              <td align="center" class="text-bold">{{x.state_code}}</td>
                              <td>{{x.state_name}}</td>
                              <td align="center">{{x.adduser}}</td>
                              <td align="center">{{$date(x.adddate, 'DD/MM/YYYY HH:mm')}}</td>
                              <td align="center">{{x.edituser}}</td>
                              <td align="center">{{$date(x.editdate, 'DD/MM/YYYY HH:mm')}}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                      ...
                      <div class="row">
                        <div class="col-md-2 col-sm-2 col-xs-2">
                          <div class="form-group">
                            <label class="text-danger">State Code</label>
                            <input type="text" class="form-control" v-model="formData.state_code" :readonly="isEdit" />
                          </div>
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-4">
                          <div class="form-group">
                            <label>State Name</label>
                            <input type="text" class="form-control" v-model="formData.state_name" />
                          </div>
                        </div>
                      </div>
                    </template>
...
<script>
  ...
  let cpn = {
    ...
    methods: {
      ...
      async onRead(x) {
        let act = `CSM/Config/State_Read?state_code=${x.state_code}`;
        let rsp = await $xt.getServer(act);
      ...
      async onSave() {
        try {
          ...
          let act = `csm/config/State_Create`;
      ...
      async onDel(x) {
        try {
          ...
          let act = `CSM/Config/State_Delete`;
          page.loadingBox.show();
          let rsp = await $xt.postServerJson(act, f);
          if (!rsp.success) {
            throw rsp.error;
    ...
    mounted() {
      page = this.$refs.page;
      page.pageTitle = 'Program Config : ตั้งค่า State';
