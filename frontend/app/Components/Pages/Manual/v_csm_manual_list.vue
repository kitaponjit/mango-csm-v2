...
<script>
  ...
  let cpn = {
    ...
    mounted() {
      ...
      this.formData.start_date = moment(new Date(new Date().getFullYear(), new Date().getMonth(), 1));
      this.formData.end_date = new Date();
      this.formData.condition = $linq(this.condition).select(x => x.key).firstOrDefault();
