    ...
    methods: {
      ...
      checkAll(e, loccode) {
        $linq(this.projPhase).foreach(x => {
          if (e.target.checked) {
            x[`chk,${x.pre_event},${loccode}`] = "Y";
          } else {
            x[`chk,${x.pre_event},${loccode}`] = "N";
          }
        });
      },
      toggleCollapse(idx) {
      ...
      async pageChange(pn) {
        ...
        this.displayData = $linq(this.projPhase).skip(paging.skipItems()).take(paging.getItemsPerPage()).toArray();
        this.collapseState = {};
