<template>
  <div>
    <modal ref="centerModal" sheet-class="ct-sheet">
      <template #header>
        <h4><i class="fa fa-th-list"></i> Project (Unit/Phase)</h4>
      </template>
      <template #body>
          ...
          <div class="item-card" v-for="x in respData" :key="x.pre_event" @click="sendData(x)">
            ...
            <div class="item-card-body">
              ...
              <div class="item-card-meta">
                ...
                <span v-if="(chk_code || 'N') == 'Y' && x.house_number"><i class="fas fa-home"></i>{{x.house_number}}</span>
              </div>
            </div>
          </div>
...
<script type="text/javascript">
  export default {
    ...
    methods: {
      ...
      loadData() {
        (async () => {
          let url = `CSM/Center/UnitPhase_ReadList?skip=${this.paging.skipItems()}&take=${this.paging.getItemsPerPage()}&pre_event2=${this.pre_event2 || ''}`;
          ...
          this.paging.setTotalItems(resp.data.total);
      ...
      initTable() {
        ...
        let fields = [
          ['pre_event', 'Project No', 'text', { width: 180, align: 'center', sortable: true, pinned: 'left', cellStyle: { 'font-weight': 'bold' } }],
          ['refcode', 'Ref.Code', 'text', { width: 160, align: 'center', sortable: true, cellStyle: { 'font-weight': 'bold' } }],
        ...
        if ((this.chk_code || 'N') == 'Y') {
          fields.push(['house_number', this.ui.erp_camp_houseno || 'House no.', 'text', { width: 200, align: 'center', sortable: true, cellStyle: { 'font-weight': 'bold' } }]);
        }
    ...
    mounted() {
      ...
      this.paging.setCurrentPage(1);
      this.paging.setItemsPerPage(10);
