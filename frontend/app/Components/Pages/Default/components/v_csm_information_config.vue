...
<script>
  ...
  methods: {
    ...
    async loadERPConfig() {
      ...
      this.displayERPConfig = $linq(this.configERPList).skip(configcode_paging.skipItems()).take(configcode_paging.getItemsPerPage()).toArray();
    ...
    loadConfigpage(){
      ...
      this.displayERPConfig = $linq(this.configERPList).where(x=> $xt.isEmpty(txt) ? true : (x.code == txt || x.remark == txt)).skip(configcode_paging.skipItems()).take(configcode_paging.getItemsPerPage()).toArray();
