...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async onSaveRemark() {
        this.form.customer_code = this.customer_code
        try {
          let f = {
            data: this.form
          };
      ...
      setProject(e) {
        this.formData.ref_pre_event = e.pre_event;
      ...
      async addNew(type,info) {
        ...
        switch (type) {
          ...
          case "license":
            ...
            // this.displayData = $linq(this.LicenseData).skip(license_paging.skipItems()).take(license_paging.getItemsPerPage()).toArray();
            license_paging.createPagesArray();
      ...
      async fileUpload(file) {
        ...
        try {
          ...
          this.form[`filepath${this.formtype}`] = r.id || "";
          this.form[`filename${this.formtype}`] = r.filename || "";
        }
        catch (ex) {
          $msg.alert('', ex.toString(), 'danger');
        }
