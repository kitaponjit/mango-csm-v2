...
export default {
  ...
  methods: {
    ...
    generateGatewayToken(token) {
      if (token === 'prod') this.formData.gateway_token_prod = $xt.generateRandomString(100);
      else if (token === 'demo') this.formData.gateway_token_demo = $xt.generateRandomString(100);
    },
    domainCount(obj) {
      let count = 0;
      for (let i = 1; i <= 6; i++) {
