...
<script>
  ...
  let cpn = {
    ...
    methods: {
      ...
      pickStatus(v) {
        this.retrieveSearch.job_status = v
        this.doSearch()
      },
      doSearch() {
      ...
      openModal(x) {
        ...
        var job_no = this.formData.job_no = x.job_no
        this.formData.job_status = x.job_status
      ...
      changeCompleteDate(parent, field, update) {
        parent[update] = parent[field] == 'Y' ? new Date() : null
      },
      priorityName(code) {
    ...
    async mounted() {
      ...
      this.retrieveSearch.job_status = this.status[0].value || ''
