...
<script>
  export default {
    ...
    computed: {
      canUpload() {
        return (this.formData.request_empno == this.auth.empno && (this.formData.job_status != 'Y' || this.editDetailData.status != 'Y') && !this.isView) || this.isAdmin;
      },
    },
    methods: {
      ...
      copyFileToClipboard(src, filename, type, btnEl) {
        var self = this;
        if (type === 'image') {
          // สำหรับรูปภาพ: fetch แล้ว copy เป็น image/png ลง clipboard
          fetch(src)
            .then(function (r) { return r.blob(); })
            .then(function (blob) {
              // แปลงเป็น PNG เสมอเพราะ clipboard API รองรับ image/png
          ...
          // สำหรับไฟล์อื่น: copy URL เป็น text ให้ paste ได้
