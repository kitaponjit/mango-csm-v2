  ...
  export default {
    ...
    methods: {
      ...
      async selectRow_header(h, d, itm) {
        $linq(h.group_contract).where(x => x.contract == d.contract).firstOrDefault().detail.forEach(x => x.select = $xt.isEmpty(x.job_no) ? d.select : "N")
        let isnotIn_contract = $linq(h.group_contract).where(x => x.contract != d.contract).toArray()
        isnotIn_contract.forEach(x => {
          x.select = "N"
          x.detail.forEach(x => x.select = "N")
        })
        ...
        this.selectedData = filter_contract.select == "Y" ? onlyDraft : []
      ...
      async selectRow_detail(h, d, itm) {
        ...
        h.select = isChecked == true ? "Y" : "N"
        this.selectedData = $linq(h.detail).where(x => $xt.isEmpty(x.job_no) && x.select == "Y").toArray()
