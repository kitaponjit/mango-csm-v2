    ...
    methods: {
      ...
      async editDetail(itemno, isView) {
        ...
        if (q.refitemno_war != null) {
          ...
          if (matchedItem || matchedItem2) this.warCheck = matchedItem.itemno
        } else {
          this.warCheck = null
        }
      ...
      async loadData(job_no) {
       page.loadingBox.show()
        ...
        this.detailDataCount = $linq(this.detailData).count()
        this.isRejectCount = $linq(this.detailData).where(x => ['R'].includes(x.status)).count()
        this.detailComplete = $linq(this.detailData).where(w => ['Y', 'R'].includes(w.status)).count()

        this.testerApproveCount = $linq(this.detailData).where(x => ['N', 'R'].includes(x.tester_approve) && x.status != 'Y').count()
        this.testerApproveWait = $linq(this.detailData).where(x => ['N'].includes(x.tester_approve) && x.status != 'Y').count()
        this.testerApproveReject = $linq(this.detailData).where(x => ['R'].includes(x.tester_approve) && x.status != 'Y').count()

        this.prapproveDataAccept = $linq(this.detailData).where(x => ['Y'].includes(x.approve_status)).count()
        this.prapproveDataReject = $linq(this.detailData).where(x => ['C'].includes(x.approve_status)).count()
        this.isNewApprove = $linq(this.detailData).any(x => this.isApprove(x.item_type))
        ...
        if (['Y'].includes(this.formData.job_status)) {
          ...
          if (resp.data.qc.length > 0) {
            this.qc_form_code = $linq(resp.data.qc).select(x => x.qaform).firstOrDefault()
      ...
      async loadCustomerWarranty() {
        try {         // console.log('ddfdfdscgdfgkiofghriouh',d)
          ...
          $linq(rsp.data).foreach(x => {
            ...
            // x.overdueY = overdue.years <= 0 || $xt.isEmpty(overdue.years) || isNaN(overdue.years) ? '' : overdue.years + ' ปี '
            ...
            // x.vendorY = v.years <= 0 || $xt.isEmpty(v.years) || isNaN(v.years) ? '' : v.years + ' ปี '
            // x.vendorM = v.months <= 0 || $xt.isEmpty(v.months) || isNaN(v.months) ? '' : v.months + ' เดือน '
            // x.vendorD = v.days <= 0 || $xt.isEmpty(v.days) || isNaN(v.days) ? '' : v.days + ' วัน '
      ...
      async setContractData() {
        ...
        this.formData.contract_user = contact != null ? contact.person_name : ''
        this.formData.contract_pos = contact != null ? contact.position : ''
        this.formData.phone = contact != null ? contact.phone : ''
        this.formData.email = contact != null ? contact.email : ''
      },
      ...
      async delFile(itemno, item_type) {
        ...
        this.attachmentData = $linq(this.attachmentData).where(x => !(x.itemno == itemno && x.ref_itemno == this.editDetailData.itemno && x.item_type == item_type)).toArray()
      },
