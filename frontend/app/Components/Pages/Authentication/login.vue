    ...
    methods: {
      ...
      loadCompany() {
        (async () => {
          ...
          this.form.maincode = $linq(resp.data).select(x => x.maincode).firstOrDefault() || '';
        })();
      },
      loadExtension() {
        (async () => {
          let resp = await $xt.getServer(`api/public/Extension_ForCallCenter`);
          this.extension = resp.data;
        })();
      },
        ...
        try {
          ...
          if (resp.data && resp.data.length > 0) {
            ...
            if (bgCSM && bgCSM.phi_path) {
              ...
              this.bgImage = window.dataServer + 'Api/File/DownLoad?id=' + bgCSM.phi_path;
            } else {
