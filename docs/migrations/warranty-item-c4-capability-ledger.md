...
| Paging | `skip/take`, pagination component; both main and Reference IC use 500 rows/page | list state page/max-page controls; Reference IC state uses `pageSize: 500` | `PARITY` |
...
| Reference IC | `WarrantyRefIC`, `addRefIC`, default `WarrantyGroup_ReadList`, one create per selected row, modal closes in `finally` | `reference-ic/warranty-item-reference-ic-service.ts`, state/controller settle callback, page workflow, display and lifecycle tests | `PARITY` |
...
| Audit formatting | legacy `datetime` cells use `DD/MM/YYYY HH:mm:ss` | grid adapter passes raw add/edit timestamps to the shared datetime renderer | `PARITY` |
...
- Reference IC: `frontend/test/warranty-item/reference-ic/warranty-item-reference-ic-service.test.ts`, `warranty-item-reference-ic-state.test.ts`, `warranty-item-reference-ic-display.test.ts`, `warranty-item-reference-ic-page-contract.test.ts`.
