  ...
  let cpn = {
    ...
    methods: {
        ...
        this.$nextTick(() => {
          agr.$el.addEventListener('input', (e) => {
            ...
            if (inp) {
              ...
              if (row) this.$set(row, 'revision_prod', digit);
            }
          });
          ...
          agr.$el.addEventListener('click', (e) => {
            ...
            let row = this.datalist.find(d => d.job_no === job_no);
            if (row) this.$set(row, 'revision_prod', this.defaultRevision);
      ...
      setDefaultRevision() {
        ...
        q.forEach(x => this.$set(x, 'revision_prod', this.defaultRevision));
        this.refreshTable();
        $notify.success(`กำหนด Revision No. ${this.defaultRevision} ให้ ${q.length} รายการ`);
      },

      clearRevision() {
        this.filteredList.forEach(x => this.$set(x, 'revision_prod', ''));
        this.refreshTable();
      },
