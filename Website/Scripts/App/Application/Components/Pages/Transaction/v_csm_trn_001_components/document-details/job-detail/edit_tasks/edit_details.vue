    ...
    methods: {
      ...
      async runAiCompareDefect() {
        ...
        try {
          ...
          this.$set(this.editDetailData, 'ai_compare_defect', JSON.stringify(rsp.data))
        } finally {
      ...
      setGaugeProgress(pct) {
        ...
        this.$set(this.editDetailData, 'n_progress', n)
      ...
      sendComponent1(e, type) {
        if (type === 'emp_work') {
          ...
          if (wk) {
            this.$set(wk, 'empno', e.empno);
            this.$set(wk, 'empfullname', e.empfullname);
          } else {
