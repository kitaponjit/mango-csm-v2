    ...
    <div class="gm-attach-grid">
      <div class="gm-attach-card" v-for="(x, idx) in attachFileTab()" :key="idx">
        ...
        <div class="gm-attach-desc">
          <textarea class="form-control input-sm gm-attach-textarea" rows="3" v-model.trim="x.description"
            v-bind:readonly="((x.add_user != auth.userid || isView) && editDetailData.tester_empno != auth.empno) && !isAdmin" placeholder="Add description..."></textarea>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      ui: window.ui,
      xt: $xt,
      lightbox: { show: false, type: '', src: '', filename: '' },
      lbEl: null,
      toastMsg: '',
  ...
  computed: {
    canUpload() {
      return ((((this.formData.job_status != 'Y' || this.editDetailData.status != 'Y') && !this.isView) || this.editDetailData.tester_empno == this.auth.empno) && this.isSoftwareTester) || this.isAdmin;
    },
  },
  methods: {
    ...
    closeLightbox() {
      ...
      if (this.lbEl) {
        ...
        if (this._lbWheel) { this.lbEl.removeEventListener('wheel', this._lbWheel); this._lbWheel = null; }
        setTimeout(() => {
          if (this.lbEl && this.lbEl.parentNode) { this.lbEl.parentNode.removeChild(this.lbEl); }
          this.lbEl = null;
        }, 250);
      }
      if (this._lbMouseMove) { document.removeEventListener('mousemove', this._lbMouseMove); this._lbMouseMove = null; }
      if (this._lbMouseUp) { document.removeEventListener('mouseup', this._lbMouseUp); this._lbMouseUp = null; }
      document.body.style.overflow = '';
    },
    renderLightbox() {
      if (this.lbEl) { this.lbEl.parentNode.removeChild(this.lbEl); }
      var el = document.createElement('div');
      el.className = 'gm-lb';
      this.lbEl = el;
      var lb = this.lightbox;
      var self = this;
      el.addEventListener('click', function (e) { if (e.target === el) self.closeLightbox(); });
      ...
      copyBtn.addEventListener('click', function () { self.copyFileToClipboard(lb.src, lb.filename, lb.type, copyBtn); });
      ...
      if (lb.type === 'image') {
        ...
        var minZoom = 0.25, maxZoom = 8, badgeTimer = null;
        ...
        function showBadge() {
          ...
          badgeTimer = setTimeout(function () { zoomBadge.classList.remove('gm-lb-zoom-badge--show'); }, 1200);
        }
        ...
        imgWrap.addEventListener('mousedown', function (e) {
          ...
          imgWrap.style.cursor = 'grabbing'; e.preventDefault();
        });
        ...
        video.className = 'gm-lb-video'; video.src = lb.src; video.controls = true; video.autoplay = true;
        content.appendChild(video);
      } else if (lb.type === 'pdf') {
        content.classList.add('gm-lb-content--doc');
        var iframe = document.createElement('iframe');
        iframe.className = 'gm-lb-iframe'; iframe.src = lb.src; iframe.setAttribute('allowfullscreen', 'true');
      ...
      this._lbEsc = function (e) { if (e.key === 'Escape') self.closeLightbox(); };
      document.addEventListener('keydown', this._lbEsc);
      requestAnimationFrame(function () { el.classList.add('gm-lb--visible'); });
    },
  },
  beforeDestroy() {
    this.closeLightbox();
