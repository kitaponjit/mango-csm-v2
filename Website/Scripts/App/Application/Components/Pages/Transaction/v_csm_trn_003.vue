  ...
  export default {
    ...
    methods: {
      ...
      async sendComponent(e, type) {
        switch (type) {
          case 'project':
            ...
            this.$set(this.header, 'customer_code', e.customer_code)
            this.$set(this.header, 'customer_name', e.customer_name)
            break
          case 'cm_customer':
            this.$set(this.header, 'customer_code', e.customer_code)
            this.$set(this.header, 'customer_name', e.name_th)
      ...
      moduleChange() {
        ...
        if (['RE', 'QCM', 'CSM', 'PPN', 'TS'].includes(this.editDetailData.module)) this.$set(this.editDetailData, 'platform', 'WEB')
      ...
      async isCheckSave() {
        if ($xt.isEmpty(this.header['pre_event'])) {
          ...
          if (this.detail.length === 1) {
            //กรณี 1 module ///
            this.$set(this.header, 'subject', `สอบถามการใช้งาน ${this.detail[0].module}`)
          } else {
            ...
            this.$set(this.header, 'subject', `สอบถามการใช้งาน ${modules}`)
          }
      ...
      clearFrom() {
        ...
        $linq(field).foreach(x => this.$set(this.header, x, null))
        this.header.job_date = new Date()
