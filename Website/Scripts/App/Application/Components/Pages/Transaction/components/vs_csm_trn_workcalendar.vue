<template>
  <div>
    <modal ref="centerModal" sheet-class="ct-sheet">
      <template #header>
        <h4><i class="ion ion-calendar"></i> Work Schedule</h4>
      </template>
      <template #body>
        <div class="wc-page">

          <div v-if="loading" class="wc-loading-overlay">
            <div class="wc-spinner"></div>
          </div>

          <div class="cal-toolbar">
            <div class="cal-search-group">
              <span class="cal-search-label">{{ ui.csm_trn_worker || 'Worker' }}</span>
            ...
            <div class="cal-right">
              ...
              <div class="docs-body">
                <div v-if="eventEmployee.length === 0" class="docs-empty">
                  ...
                  <p>{{ ui.erp_data_not_found || 'ไม่พบข้อมูล' }}</p>
                </div>
                <div class="doc-item" v-for="(item, index) in eventEmployee" :key="index">
                  ...
                  <div class="doc-item-rows">
                    <div class="doc-item-row">
                      <span class="doc-key">{{ ui.erp_request || 'Request' }}</span>
                  ...
                  <div class="doc-item-foot">
                    ...
                    <span v-if="item.overdate > 0" class="doc-overdue">
                      เกินกำหนด {{ item.overdate }} {{ ui.erp_day || 'วัน' }}
                    </span>
                  </div>
                </div>
              </div>
...
import dayGridPlugin from '@fullcalendar/daygrid';
  let cpn = {
    ...
    methods: {
      openModal() {
        (async () => {
          ...
          await this.loadCheckDateWorker();
        })();
      },
      closeModal() {
      ...
      async loadEmployee() {
        ...
        this.checkEmpno = this.worker_empno;
      },
      async loadCheckDateWorker() {
        this.loading = true;
        let rsp = await $xt.getServer(`csm/data/CSM_CheckWorker?empno=${encodeURIComponent(this.checkEmpno)}`);
        this.loading = false;

        this.markedDates = $linq(rsp).select(x => moment(x.due_date).format('YYYY-MM-DD')).toArray();
        this.eventEmployee = $linq(rsp).select(x => ({
          date: moment(x.due_date == null ? x.response_date : x.due_date).format('DD/MM/YYYY'),
          title: x.job_no,
          desc: x.subject,
    ...
    computed: {
      employeeOptions() {
        return [
          { id: 'All', text: this.ui.erp_all || 'All' },
...
.docs-body::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 4px; }
