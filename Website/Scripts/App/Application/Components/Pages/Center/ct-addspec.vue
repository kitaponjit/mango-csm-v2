    ...
    <modal ref="centerModal" sheet-class="ct-sheet">
      ...
      <template #body>
            ...
            <div class="col-lg-4 col-md-3">
              <div class="form-group">
                ...
                <div class="input-group">
                  <input type="text" class="form-control input-sm" v-model.trim="retrieveSearch['text']"
                    @keyup.enter="onSearch" />
                  <span class="input-group-btn"><a class="btn btn-sm btn-default" @click="onSearch"><i
        ...
        <div class="view-toolbar">
          <label class="spec-select-all">
            <input type="checkbox" true-value="Y" false-value="N" v-model="selectAllChecked" @change="selectAll" />
            <span>{{ ui.erp_select_all || 'เลือกทั้งหมด' }}</span>
          </label>
        ...
        <div class="card-grid" v-show="viewMode === 'card'">
          <div class="item-card spec-card" v-for="(x, idx) in displayData" :key="x.itemno">
            <div class="spec-card-checkbox">
              <input type="checkbox" true-value="Y" false-value="N" v-model="x.checked" @change="selectItem(displayData, x, idx)" />
            </div>
      ...
      <template #footer>
        ...
        <button type="button" class="btn btn-sm btn-primary modal-close-btn" @click="sendSelectedItems">
          <i class="fa fa-check"></i> {{ ui.erp_select || 'Select' }}
        </button>
      </template>
    </modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      let module = this.retrieveSearch['module']
      let text = this.retrieveSearch['text']
      let field = this.retrieveSearch['field']
      let url = `AnywhereAPI/Master/Addspec_Object_ReadList?module=${module}&field=${field}&text=${text}`;
    ...
    sendData(x) {
      ...
      this.closeModal();
      console.log('this.objecttype:', this.retrieveSearch.field);
    },
    objectType(type) {
      let d = ""
      switch (type) {
    ...
    toggleSelection(item, idx) {
      this.$set(this.displayData[idx], "checked", !item.checked == "Y" ? "Y" : "N")
      this.$set(this, "selectedItems", this.displayData.filter(x => x.checked == "Y"))
    },
    sendSelectedItems() {
      this.$emit("send-data", this.selectedItems);
      this.closeModal();
    },
    selectItem(_, item) {
      let findIndex = this.datalist.findIndex(x => x.module == item.module && x.object_name == item.object_name)

      this.$set(this.datalist[findIndex], "checked", item.checked)
      this.$set(this, "selectedItems", this.datalist.filter(x => x.checked == "Y"))
      this.$set(this, "selectAllChecked", this.datalist.every(x => x.checked == "Y") ? "Y" : "N")
      this.pageChange(this.pageNumber)
    },
    selectAll() {
      this.selectAllChecked == "Y" ? this.selectedItems = [...this.datalist] : this.selectedItems = []
      this.datalist.forEach(x => x.checked = this.selectAllChecked == "Y" ? "Y" : "N")
      this.pageChange(this.pageNumber)
    },
    refreshGrid() {
      this.$nextTick(() => {
    ...
    initTable() {
      ...
      let fields = [
        ['checked', '', 'text', {
          width: 50, align: 'center', pinned: 'left', sortable: false,
          cellRenderer: (params) => {
            let x = params.data
            let cb = document.createElement('input')
            cb.type = 'checkbox'
            cb.checked = x.checked == 'Y'
            cb.addEventListener('change', () => {
              x.checked = cb.checked ? 'Y' : 'N'
              self.selectItem(self.displayData, x)
            })
            return cb
          }
        }],
        ['itemno', this.ui.erp_no || 'No.', 'text', { width: 80, align: 'center' }],
        ['module', this.ui.erp_module || 'Module', 'text', { width: 160 }],
        ['object_name', this.ui.erp_name || 'Name', 'text', { width: 280 }],
        ['object_type', this.ui.erp_type || 'Type', 'text', { width: 180, cellRenderer: (params) => self.objectType(params.value) }],
        ['remark', this.ui.csm_v2_description || 'Description', 'text', { width: 440 }],
      ]
