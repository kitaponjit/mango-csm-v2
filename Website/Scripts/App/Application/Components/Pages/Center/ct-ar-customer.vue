<template>
  <div>
    <modal ref="centerModal" sheet-class="ct-sheet">
      <template #header>
        <h4><i class="fa fa-building-o"></i> ข้อมูลลูกค้า (AR)</h4>
      </template>
      <template #body>
        ...
        <div class="card-grid" v-show="viewMode === 'card'">
          <div class="item-card" v-for="x in respData" :key="x.customer_code" @click="sendData(x)">
            <div class="item-card-avatar"><i class="fas fa-building"></i></div>
            <div class="item-card-body">
              <div class="item-card-top">
                <span class="item-card-code">{{x.customer_code}}</span>
              </div>
              <div class="item-card-sub" :title="x.customer_name">{{x.customer_name}}</div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      initTable() {
        ...
        let fields = [
          ['customer_code', this.ui.erp_code || 'Code', 'text', { width: 200, sortable: true, pinned: 'left' }],
          ['customer_name', this.ui.erp_customer || 'Customer', 'text', { width: 420, sortable: true }],
        ];
