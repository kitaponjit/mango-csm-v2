<template>
  <div>
    <modal ref="centerModal" sheet-class="ct-sheet">
      <template #header>
        <h4><i class="fa fa-map-marker"></i> ข้อมูลพื้นที่</h4>
      </template>
      <template #body>
        ...
        <div class="card-grid" v-show="viewMode === 'card'">
          <div class="item-card" v-for="x in respData" :key="x.loccode" @click="sendData(x)">
            <div class="item-card-avatar"><i class="fas fa-map-marker-alt"></i></div>
            <div class="item-card-body">
              <div class="item-card-top">
                <span class="item-card-code">{{x.loccode}}</span>
              </div>
              <div class="item-card-sub" :title="x.locname">{{x.locname}}</div>
            </div>
          </div>
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        (async () => {
          let act = `CSM/Center/Area_ReadList?skip=${paging.skipItems()}&take=${paging.getItemsPerPage()}&pre_event=${this.preEvent}`;
      ...
      initTable() {
        ...
        let fields = [
          ['loccode', this.ui.erp_loccode || 'รหัสพื้นที่', 'text', { width: 200, sortable: true, pinned: 'left' }],
          ['locname', this.ui.erp_locname || 'ชื่อพื้นที่', 'text', { width: 420, sortable: true }],
        ];
