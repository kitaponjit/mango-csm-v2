  ...
  let cpn = {
    ...
    methods: {
      ...
      onTableClick(e) {
        ...
        if (chk) {
          ...
          this.$set(row, 'default_', chk.checked ? 'Y' : 'N');
      ...
      async onReadData(prioity_code) {
        try {
          ...
          if ($xt.isEmpty(this.form.priority_color)) {
            this.$set(this.form, "priority_color", "#000000");
          }
     ...
     async is_mango() {
      ...
      filterInput(event) {
        this.$set(this.form, "prioity_code", event.target.value.replace(/[^a-z0-9]/gi, ''))
      },
    ...
    watch: {
      'form.to_date'(newValue) {
        ...
        this.$set(this.form, "to_date", onlyNumb )

        if (onlyNumb < 1 || $xt.isEmpty(onlyNumb) ) {
          this.$set(this.form, "to_date", 1)
        }
      }
    },
    mounted() {
