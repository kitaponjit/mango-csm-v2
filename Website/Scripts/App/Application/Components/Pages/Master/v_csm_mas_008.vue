  ...
  let cpn = {
    ...
    methods: {
      ...
      selectItem(item){
        ...
        this.$set(this.addItemDisplay[findIndex], "checked", item.checked)
        this.$set(this, "selectedItems" , this.addItemDisplay.filter(x => x.checked == "Y"))
        this.$set(this, "selectAllChecked", this.addItemDisplay.every(x => x.checked == "Y") ? "Y" : "N")
      },
      ...
      async pageChange(pn) {
        ...
        this.page_number = pn;
        paging.setCurrentPage(pn);
        this.$set(this, "dataListDisplay", $linq(this.dataList).skip(paging.skipItems()).take(paging.getItemsPerPage()).toArray());
        paging.createPagesArray();
        this.collapseState = {};
        this.agTableKey++;
      },
      ...
      this.$nextTick(() => {
        ...
        $(el).on('input', '.ag-input-cell', function() {
          ...
          if (filteredWarranty[row]) {
            self.$set(filteredWarranty[row], field, $(this).val());
          }
        });
        ...
        $(el).on('change', '.ag-date-cell', function() {
          ...
          if (filteredWarranty[row]) {
            if (val) {
              ...
              if (parsed.isValid()) {
                self.$set(filteredWarranty[row], field, parsed.toDate());
                if (field === 'startdate') {
              ...
              self.$set(filteredWarranty[row], field, null);
            }
          }
        });
        ...
        $(el).on('change', '.ag-chk-active', function() {
          let row = startIdx + parseInt($(this).data('row'));
          if (filteredWarranty[row]) {
            self.$set(filteredWarranty[row], 'active_row', $(this).is(':checked') ? 'Y' : 'N');
          }
        });
        ...
        $(el).on('click', '.ag-btn-start', function(e) {
          ...
          if (filteredWarranty[row]) {
            ...
            if (item.active === 'Y' && item._justStarted) {
              ...
              self.$set(item, 'active', 'N');
              self.$set(item, '_justStarted', false);
              self.$set(item, 'startdate', null);
              self.$set(item, 'enddate', null);
              self.refreshAreaTable(idx);
      ...
      try {
        if(dataZ.startdate != null && dataZ.enddate != null && dataZ.active === 'Y') {
          ...
          if(daysRemaining <= 0) {
            ...
            this.$set(dataZ, "startdate", enddate);
            this.$set(dataZ, "enddate", startDate);
            this.$set(dataZ, "revno", revno_count);
    ...
    reviseModal(data, index) {
      // this.tmpData = data
      this.$set(this.tmpData, 'war_code', data.item_warranty[index].war_code)
      this.$set(this.tmpData, 'serial_number', data.item_warranty[index].serial_number)
      this.$set(this.tmpData, 'loccode', data.loccode)
      this.$set(this.tmpData, 'locname', data.locname)
      
      this.$set(this.retrieveSearch, 'all', 'N')
      this.$set(this.retrieveSearch, 'text', data.item_warranty[index].revno)
    ...
    reviseSearch() {
      this.$set(this, 'reviseSelect', this.retrieveSearch.text || 0)
      ...
      if (data.item_warranty[0].startdate != null) {
        ...
        data.item_warranty.forEach(item => {
          if (item.startdate == null && item.active !== 'Y') {
            this.$set(item, "startdate", data.item_warranty[0].startdate);
            ...
            this.$set(item, "enddate", todayDate);
          }
        });
      }
