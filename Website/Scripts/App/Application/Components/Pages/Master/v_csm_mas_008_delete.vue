  ...
  let cpn = {
    ...
    methods: {
      ...
      async loadAreaData() {
        try {
          ...
          this.dataArea.forEach((x) => {
            this.$set(x, "checked", "N");
          });
          
          this.dataArea.forEach((x, index) => {
            this.$set(x, "rowno", index + 1);
          });
      ...
      async pageAreaChange(pn) {
        ...
        this.$set(this, "dataAreaDisplay", $linq(this.dataArea).skip(pagingArea.skipItems()).take(pagingArea.getItemsPerPage()).toArray());
        pagingArea.createPagesArray();
      },
      pageWarrantyChange(pn){
        ...
        if (this.area_code.length > 0) {
          ...
          this.$set(this, "dataWarrantyDisplay", $linq(this.dataWarrantyDisplay).skip(pagingWarranty.skipItems()).take(pagingWarranty.getItemsPerPage()).toArray());
        } else {
          this.$set(this, "dataWarrantyDisplay", $linq(this.dataWarranty).skip(pagingWarranty.skipItems()).take(pagingWarranty.getItemsPerPage()).toArray());
        }
        pagingWarranty.createPagesArray();
      },
      async doSearch(type) {
      ...
      selectItem(item, type){
        if (type === 'area') {
          ...
          this.$set(this.dataAreaDisplay[findIndex], "checked", item.checked)
          this.$set(this, "selectAreaItems" , this.dataAreaDisplay.filter(x => x.checked == "Y"))
          this.$set(this, "selectAllArea", this.dataAreaDisplay.every(x => x.checked == "Y") ? "Y" : "N")
          this.pageAreaChange(this.areaPageNumber)
        } else if (type === 'warranty') {
          ...
          this.$set(this.dataWarrantyDisplay[findIndex], "checked", item.checked)
          this.$set(this, "selectWarrantyItems" , this.dataWarrantyDisplay.filter(x => x.checked == "Y"))
          this.$set(this, "selectAllWarranty", this.dataWarrantyDisplay.filter(a => a.active !== 'Y').every(x => x.checked == "Y") ? "Y" : "N")
          this.pageWarrantyChange(this.warrantyPageNumber)
        }
      },
      async selectArea() {
