  ...
  let cpn = {
    ...
    methods: {
      ...
      async pageChange(pn) {
        ...
        this.$set(this, "displayData", $linq(this.datalist).where(x => x.hdtype == "H").skip(paging.skipItems()).take(paging.getItemsPerPage()).toArray());
          ...
          agr.$el.addEventListener('input', (e) => {
            ...
            if (codeInput) {
              ...
              if (this.detailData[idx]) {
                this.$set(this.detailData[idx], 'serv_code_d', codeInput.value);
              }
            }
            if (remarkInput) {
              let idx = parseInt(remarkInput.getAttribute('data-idx'));
              if (this.detailData[idx]) {
                this.$set(this.detailData[idx], 'remark', remarkInput.value);
              }
            }
          });
        });
