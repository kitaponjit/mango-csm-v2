  ...
  export default {
    ...
    methods: {
      ...
      async readImg() {
        ...
        this.$set(this, 'image_bg', resp.data || []);
      },
      async loadConfig() {
        let action = `CSM/API/GetConfig`
        let data = await $xt.getCustomerServer(action);
        await this.$set(this, 'is_mango', data.config_value);
        await this.$set(this, 'maincode_', data.maincode);
      },
      setBackground() {
        this.$nextTick(() => {
      ...
      async requestOtp() {
        ...
        try {
          ...
          this.otp_ref = ''
          this.$set(this.forgetForm, 'otp_pass', '')
    ...
    async mounted() {
      ...
      if (logincust != null || logincust != undefined) {
        ...
        this.$set(this.form, 'userid', p.userid);
