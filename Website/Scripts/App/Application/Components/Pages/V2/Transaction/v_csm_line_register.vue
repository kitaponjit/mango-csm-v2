...
export default {
  ...
  methods: {
    ...
    async LineOA_CreateUser(){
      this.$refs.myLB.show()
      try {
        this.$set(this.form, "userid", this.form.userid.toLowerCase())
        let obj = {
          data : this.form
        }
    ...
    onlyPressNumber() {
      ...
      this.$set(this.form , "telephone", this.form.telephone.replace(/[^0-9]/g, ''))
      // this.form.contract_tel = this.form.contract_tel.replace(/[^0-9]/g, '');
    },
    filterInput(event) {
      this.$set(this.form, "userid", event.target.value.replace(/[^a-z0-9]/gi, ''))
    }
