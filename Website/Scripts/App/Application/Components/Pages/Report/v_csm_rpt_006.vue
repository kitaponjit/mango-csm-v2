...
export default {
  ...
  methods: {
    async setRptData(d, cond) {
      ...
      this.$set(this, "display", d.result);
  ...
  async mounted() {
    ...
    await $xt.sleep(500);
    $linq(this.condTemplate).where((x) => x.field_name == "module").foreach((x) => {this.$set(x, "value_arr", $linq(this.moduleForMango).select((x) => { return { value: x, name: x, }; }).toArray());});

    $linq(this.condTemplate).where((x) => x.field_name == "req_type").foreach((x) => {this.$set(x, "value_arr", $linq(this.requestCodeData).select((x) => { return { value: x.req_code, name: x.req_des } }).toArray())});

    $linq(this.condTemplate).where((x) => x.field_name == "item_type").foreach((x) => {this.$set(x, "value_arr", $linq(this.serviceCodeData).select((x) => { return { value: x.serv_code, name: x.serv_name } }).toArray())});

    $linq(this.condTemplate).where((x) => x.field_name == "connection_type").foreach((x) => {this.$set(x, "value_arr", $linq(this.connectionCodeData).select((x) => { return { value: x.contact_code, name: x.contact_name,}}).toArray())});

    $linq(this.condTemplate).where((x) => x.field_name == "status").foreach((x) => {this.$set(x, "value_arr", $linq(this.statusCodeData).select((x) => {return { value: x.id, name: x.name}}).toArray())});

    $linq(this.condTemplate).where((x) => x.field_name == "platform").foreach((x) => {this.$set(x, "value_arr", $linq(this.platformCodeData).where((x) => x.id != "111").select((x) => {return {value: x.id, name: x.name}}).toArray())});

    $linq(this.condTemplate).where((x) => x.field_name == "object_type").foreach((x) => {this.$set(x, "value_arr", $linq(this.job_type).select((x) => {return {value: x.id,name: x.name}}).toArray() )});
