  ...
  let cpn = {
    ...
    methods: {
      ...
      async loadAll() {
        ...
        this.$set(this, "shownCond", { year: this.cond.year, month: this.cond.month });
        try {
      ...
      async loadRptData() {
        ...
        $linq(this.rptHeader).foreach(x => this.$set(x, 'checked', x.projtype == 'ALL'));
      },
      loadChart() {
      ...
      selectProj(x) {
        $linq(this.rptHeader).foreach(f => this.$set(f, 'checked', f.projtype == x.projtype));
        ...
        this.$nextTick(() => requestAnimationFrame(() => this.$set(this, 'fadein', true)));
      }
    },
    mounted() {
      page = this.$refs.page;
