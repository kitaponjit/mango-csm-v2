              ...
              <div class="cond-card-fields">
                ...
                <div class="cond-field cond-field-val">
                  ...
                  <template v-else-if="x.option.field_type === 'number'">
                    <i-input input-class="form-control input-sm" v-model.number="x['value']" :number-only="true" :disabled="(x.operatorx == 'is null' || x.operatorx == 'is not null')" v-bind:readonly="!x['field_name']" @keyup="$set(x,'display_value',x['value'])"></i-input>
                  </template>
                  <!-- Data Type : Date -->
...
<script>
  ...
  let vue = {
    ...
    methods: {
      ...
      setTemplate(tmp) {
        for (var x of tmp) {
          this.$set(x, 'not_input', x?.not_input || false)
        }
      ...
      addOption(x) {
        ...
        this.$set(x, 'display_value', '')
        this.$set(x, 'multiple_type', option.multiple_type || 'and')
      ...
      //        this.$set(this.printSetup, 'page_layout', this.pageLayout)
      //      }
