...
const activityHandler = () => {
    ...
    methods: {
      // ─── UI ──────────────────────────────────────────────────────────────────
            ...
            async loadState() {
              ...
              this.$set(this, 'stateData', rsp.data.data_rows.filter(x => !['11', '12'].includes(x.state_code)))
            },
            async setState() {
            ...
            async loadCSM_Line() {
              ...
              this.$set(this, 'totalCSMLine', rsp.total)
            },
            // ─── Document Popup ───────────────────────────────────────────────────────
            popupDocument() {
          ...
          try {
            this.$set(this, 'isLoadingCompany', true)
            ...
            this.$set(this, 'selectedMaincode', this.auth.maincode)
          } catch (ex) {
            $msg.alert(this.ui.csm_v2_warning, ex, 'warning')
          } finally {
            this.$set(this, 'isLoadingCompany', false)
          }
        },
      async changeCompany() {
  ...
  async checkUserOnline2() {
    ...
    try {
      ...
      this.$set(this, 'onlineUser', sessionData.onlineUser);
  ...
  async handleBroadcastAction({ action, data }) {
    if (action === 'update') {
      if (data.onlineUser) this.$set(this, 'onlineUser', data.onlineUser);
